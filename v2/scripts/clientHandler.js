...
export default class clientHandler {
    ...
    constructor(entityId,entityElemId,fieldName,domain,inputElement,inputParent,tag,clear,loader,btnElement) {
        ...
        this.valueLength = entityId=="CRM_CONTACT"?"14":"9"; // Вставляеться в название метода при вызове( crm.company.add./crm.contact.add и тд)
    ...
    async processInput(target)
    {
        ...
        {
            if(
                (this.type=="entity" && inn.length == this.valueLength) ||
                (this.type=="individual" && inn.length == this.valueLength) || 
                ...
                if(dataApi!=null && isNaN(dataApi)){
                    if(companyIdWithInn==false){ // создание 
                        ...
                        //обновление карточки , тк id совпадает с id текущей сущности
                            ...
                            crmRequisiteAdd: {
                                ...
                                params: {
                                    fields:{ 
                                        ...
                                        "UF_CRM_RQ_PINFL" : ENTITY=="contact"?arDataGnk.gnk_company_director_pinfl:arDataGnk.company_tin,
