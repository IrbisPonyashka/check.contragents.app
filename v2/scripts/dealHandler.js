export default class DealCompanyHandler {
    ...
    async processInputInDeal(target) { // процессе события input буду происходить только получение данных и установка конкретного статуса(действий) для события change 
        ...
        if(this.entityElemId==0){
            ...
            if(inn.length==9 || inn.length==14 && this.isValidInn(inn) ){
                ...
                const type = inn.length==14 ? "contact" : "company";
                                ...
                                crmRequisiteAdd: {
                                    ...
                                    params: {
                                        fields:{ 
                                            ...
                                            "RQ_INN" : type=="contact"?arDataGnk.gnk_company_director_pinfl:arDataGnk.company_tin,
    ...
    {
        // company = await this.getCompanyIdById(companyId);
        ...
        return new Promise((resolve, reject) => {
            ...
            var objFields = {};
            if(type=="contact"){
                objFields.CONTACT_ID=companyId,  
                objFields[this.fieldName] = arDataGnk.gnk_company_director_pinfl;
            }else{
                objFields.COMPANY_ID=companyId,  
                objFields[this.fieldName] = arDataGnk.company_tin;
            }
    ...
    dealHasCompany(dealId, companyId, inn) {
